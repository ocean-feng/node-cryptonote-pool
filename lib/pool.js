...
function Miner(id, login, minername, pass, ip, startingDiff, pushMessage){
    this.id = id;
    // add worker support login = [xmr_address.PaymentID.workername]
    // keep login to []
    this.login = login;
...
Miner.prototype = {
    retarget: function(now){
        if(config.poolServer.varDiff.enabled)
            return;
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    var minerfull = miner.login;
    if(miner.minername)
        minerfull += '@' + miner.minername;
    var redisCommands = [
        ['hincrby', config.coin + ':shares:roundCurrent', miner.login, job.score],
        ['zadd', config.coin + ':hashrate', dateNowSeconds, [job.difficulty, minerfull, dateNow].join(':')],
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
            ...
            var login;
            var minername;
            var logininfo = params.login.split('@');
            if(logininfo.length === 1)
                login = logininfo[0];
            else if(logininfo.length === 2){
                login = logininfo[0];
                minername = logininfo[1];
            }else{
                sendReply('invalid address used for login');
            }
            var minerId = utils.uid();
            miner = new Miner(minerId, login, minername, params.pass, ip, portData.difficulty, pushMessage);
            ...
            if(minername)
                log('info', logSystem, 'Miner connected %s@%s@%s',  [login, minername, miner.ip]);
            else
                log('info', logSystem, 'Miner connected %s@%s',  [login, minername, miner.ip]);
            break;
        case 'getjob':
            if (!miner){
