    ...
    async.parallel({
        pool: function(callback){
            redisClient.multi(redisCommands).exec(function(error, replies){
                ...
                minerStats = {};
                addressStats = {};
                addressLastShareTime = {};

                for (var i = 0; i < hashrates.length; i++){
                    var hashParts = hashrates[i].split(':');
                    var address = hashParts[1].split('@');
                    minerStats[address[0]] = (minerStats[address[0]] || 0) + parseInt(hashParts[0]);
                    // if(address.length === 2){
                    addressStats[hashParts[1]] = (addressStats[hashParts[1]] || 0) + parseInt(hashParts[0]);
                    addressLastShareTime[hashParts[1]] = hashParts[2] / 1000;
                    // }
                    // else{
                    //     addressStats[hashParts[1]] = (addressStats[hashParts[1]] || 0) + parseInt(hashParts[0]);
                    // }
                ...
                for (var address in addressStats){
                    var shares = addressStats[address];
                    var workerinfo = {};
                    workerinfo['hashrate'] = getReadableHashRateString(shares / config.api.hashrateWindow);
                    workerinfo['lastsharetime'] = addressLastShareTime[address];
                    addressStats[address] = workerinfo;
                }

                // log('error', logSystem, 'addressStats %j', [addressStats]);

                data.miners = Object.keys(addressStats).length;
...
function handleWorkerStats(urlParts, response){
    ...
    redisClient.exists(config.coin + ':workers:' + address, function(error, result){
        ...
        var workerStats = {};
        for(worker in addressStats){
            if(worker.indexOf(address) > -1 ){
                workername = worker.split('@');
                if(workername.length === 2)
                    workerStats[workername[1]] = addressStats[worker];
                else
                    workerStats[worker] = addressStats[worker];
            }
        }
        // log('error', logSystem, 'miner username = %d %s', [addressStats[address + "*"], address]);
        response.end(JSON.stringify({worker: workerStats}));
    });
}
